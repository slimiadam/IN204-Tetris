...
//Déclaration des variables
    ...
    Shape::Ptr piece = Shape::Ptr(new J());
    Shape::Ptr next_piece = Shape::Ptr(new J());
...
// Déclaration des variables réseaux
...
// Affichage des scores
    ...
    while(keep)
    {
        ...
        {   piece = next_piece;
            //Reception de la pièce via le serveur
            char buffer[512] = "cant";
            //Reception de la pièce via le serveur
            SDLNet_TCP_Send(client, (void *)buffer, strlen(buffer) + 1);
            SDLNet_TCP_Recv(client,buffer, 512);
            std::cout << "J'ai recu une nouvelle piece du serveur !" << buffer << std::endl;
            piece = new_piece(buffer[0]);
            game_grid.delete_line();
            bool can = game_grid.add_piece(piece);
            game_grid.delete_line();
            bool can = game_grid.add_piece(piece);
            int len = sizeof(int) + 1;
            if (SDLNet_TCP_Send(client, &game_grid._score, len) < len) {
             std::cout << "SDLNet_TCP_Send: " << SDLNet_GetError() << std::endl; 
             exit(EXIT_FAILURE); }

        
            // //Reception du score du deuxième joueur 
            SDLNet_TCP_Recv(client, &second_grid._score, len);
            if (not(can)){
                //Envoie message de fin au serveur
                int len = sizeof(int) + 1;
                char buffer[512] = "quit";
                if (SDLNet_TCP_Send(client, (void *)buffer, strlen(buffer) + 1) < strlen(buffer) + 1) {
                    std::cout << "SDLNet_TCP_Send: " << SDLNet_GetError() << std::endl; 
                    exit(EXIT_FAILURE); }
            ...
            char buffer[512] = "can";
            SDLNet_TCP_Send(client, (void *)buffer, strlen(buffer) + 1);
            std::cout << "J'ai envoyé can au serveur !" << std::endl;
        }
        }
